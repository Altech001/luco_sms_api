...
@pytest.fixture
def client():
    return LucoSMSClient(api_key="test_api_key", base_url="http://test.api")
...
def test_send_sms(client, mock_api):
    ...
    mock_api.post("http://test.api/api/v1/client/send-sms", json=mock_response)
...
def test_get_balance(client, mock_api):
    ...
    mock_api.get("http://test.api/api/v1/wallet-balance", json=mock_response)
...
def test_get_delivery_report(client, mock_api):
    ...
    mock_api.get("http://test.api/api/v1/delivery_report", json=mock_response)
...
def test_get_sms_history(client, mock_api):
    ...
    mock_api.get("http://test.api/api/v1/sms_history", json=mock_response)
...
def test_invalid_api_key(mock_api):
    ...
    mock_api.post(
        "http://localhost:8000/api/v1/client/send-sms",
        status_code=401,
        json={"detail": "Invalid API key"}
    )
    
    with pytest.raises(requests.exceptions.HTTPError):
        client.send_sms(recipient="+256700000000", message="Test message")

def test_server_error(client, mock_api):
    mock_api.post(
        "http://test.api/api/v1/client/send-sms",
        status_code=500,
